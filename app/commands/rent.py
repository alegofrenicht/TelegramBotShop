...
def rent(message):
    global categories
    bot.send_message(message.chat.id, text_templates.rent_text, parse_mode=telegram.constants.ParseMode.HTML)
    with app.app_context():
        devices = models.Device.query.filter_by(placement_type='rent').all()
        ...
        if not devices:
            ...
            for category in categories:
                button = types.InlineKeyboardButton(f"{category.capitalize()}", callback_data=f'category for rent:{category.lower()}')
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('category for rent:'))
def category_func(call):
    ...
    with app.app_context():
        ...
        devices = models.Device.query.filter_by(placement_type='rent', device_type=category.capitalize())
...
def deal(call):
    ...
    with app.app_context():
        ...
        bot.send_message(owner.chat_id, f'{emoji}{text_templates.deal_text.format(call.from_user.username, device.name)}')
        bot.send_message(call.message.chat.id, f'{emoji}Your request was just sent to the owner. They will get in touch with you to discuss further details ')
